...
/**
 * [KSerializer] for `Map<Int, ObservablePlaylist>` used to round-trip a JavaFX playlist hierarchy
 * through JSON.
 *
 ...
 * (`Int`/`String`/`Boolean`) plus aggregate-id lists, so no contextual serializers module is required.
 * The sample instance is built construction-free through the playlist's co-located
...
private fun sampleObservablePlaylist(): ReactiveEntityBase<Int, ObservablePlaylist> =
    rawConstruct(
        "net.transgressoft.commons.fx.music.playlist.FXPlaylist",
        mapOf(
            "id" to 0,
            "name" to "",
