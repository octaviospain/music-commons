 ...
 *
 * The mutable audio item's constructor is `internal` to the reactive base module, so this mapping
 ...
 * [toParams] writes the row using the relocated `ColumnConverter`s, and [applyScalarRow] populates the
 * reactive and non-constructor fields after construction via the entity's `LirpRawInitializer`
 * silent-setter path. There is no `fromRow` override — construction is delegated entirely to lirp.
 *
...
internal object MutableAudioItemSqlTableDef : RawConstructibleTableDef<AudioItem> {
    ...
    override val entityClassName: String = "net.transgressoft.commons.music.audio.MutableAudioItem"
    ...
    override fun applyScalarRow(
        ...
        row: ResultRow,
        table: Table,
        rawInit: LirpRawInitializer<AudioItem>
    ) {
        val column = column(table)
