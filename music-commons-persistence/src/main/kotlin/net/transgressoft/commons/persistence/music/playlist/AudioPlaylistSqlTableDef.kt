...
/**
 * Construction-free [RawConstructibleTableDef] mapping the core playlist entity to a SQLite table.
 *
 * The mutable playlist's constructor is `internal` to the reactive base module, so this mapping
 * never constructs the entity itself: [entityClassName] points lirp's SQL loader at the entity's
 * co-located `_LirpRawConstructor`, which it resolves reflectively and feeds with [constructorParams].
 * The scalar reactive fields (`name`, `isDirectory`, `lastDateModified`) are re-applied after
 * construction via the entity's `LirpRawInitializer` silent-setter path in [applyScalarRow]. There
 * is no `fromRow` override — construction is delegated entirely to lirp.
 *
 ...
 * afterward, because the `mutableAggregateList`/`mutableAggregateSet` delegates capture their backing
 * IDs from the constructor arguments — so the parsed IDs are passed through [constructorParams] as
 * the `initialAudioItemIds`/`initialPlaylistIds` keys the entity's `_LirpRawConstructor` expects.
 *
 * Column order and the `audio_playlist` table name are stable.
 * @since 1.0
 */
public object AudioPlaylistSqlTableDef : RawConstructibleTableDef<MutableAudioPlaylist> {

    override val tableName: String = "audio_playlist"

    override val entityClassName: String = "net.transgressoft.commons.music.playlist.MutablePlaylist"
    ...
    override fun applyRow(entity: MutableAudioPlaylist, row: ResultRow, table: Table) {
        ...
        // audio_item_ids/playlist_ids back the `audioItems`/`playlists` aggregate delegates (val),
    ...
    override fun applyScalarRow(
        entity: MutableAudioPlaylist,
        row: ResultRow,
        table: Table,
        rawInit: LirpRawInitializer<MutableAudioPlaylist>
    ) {
        val column = column(table)
