...
internal object FXAudioItemTestUtil : TestConfiguration() {
    ...
    fun arbitraryArtist(givenName: String? = null, countryCode: CountryCode? = null): Arb<Artist> = arbitrary {
        ImmutableArtist.of(givenName ?: beautifyArtistName(Arb.stringPattern("[a-z]{5} [a-z]{5}").bind()), countryCode ?: CountryCode.entries.toTypedArray().random())
    }
    ...
    fun arbitraryAudioItem(attributesAction: AudioItemTestAttributes.() -> Unit): Arb<FXAudioItem> = arbitrary {
        val attributes = arbitraryAudioAttributes().bind()
