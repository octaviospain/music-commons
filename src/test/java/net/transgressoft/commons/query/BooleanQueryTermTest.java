...
class BooleanQueryTermTest {
    ...
    void twoTermsQuery() {
        var query = QueryPredicate.of(
                HEIGHT.isGreaterThan(1.70f)).and(NAME.notContains("Leia"));

        assertThat(query.apply(entity)).isTrue();

        query = QueryPredicate.of(
                BIRTH_DATE.isAfter(LocalDateTime.of(1985, Month.JANUARY, 1, 1, 1, 1))
                        .and(NAME.contains("Keno")));

        assertThat(query.apply(entity)).isTrue();

        query = QueryPredicate.of(
                INTERESTS.isGreaterThan((short) 1)
                .and(PersonIntegerAttribute.MONEY.isLessThan(1_000_000)));

        assertThat(query.apply(entity)).isTrue();

        query = QueryPredicate.of(
                BREATH_DURATION.isLongerThan(ofSeconds(60))
                .and(entity -> entity.id() == 23));

        assertThat(query.apply(entity)).isTrue();

        query = QueryPredicate.of(
                BREATH_DURATION.notEqualsTo(ofSeconds(500))
                .and(entity -> entity.getUniqueId().equals("23-Obi Wan Kenobi-1.75")));
